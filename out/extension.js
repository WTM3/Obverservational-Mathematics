...
function activate(context) {
    ...
    vscode.window.showInformationMessage('🏁 BLF NJSON V-8 Engine ready! The narrow bridge between chaos and control is active.', 'Show Commands').then(selection => {
...
function updateEngineConfig() {
    ...
    const options = {
        bufferValue: config.get('bufferValue', 0.1),
        maxInputSize: config.get('maxInputSize', 15000),
        heatShieldEnabled: config.get('heatShieldEnabled', true)
    };
    blfEngine = new blfEngine_1.BLFNJSONEngine(options);
}
...
🎯 The V-8 engine stands ready to process Boolean Language Framework queries.`;
    ...
    vscode.window.showInformationMessage(`${status.status} | Processing Count: ${status.processingCount} | Heat Shield: ${status.heatShield}`, 'Show Details').then(selection => {
...
Detailed Results:
${validation.testResults.map(test => `  • "${test.input}" | AIC: ${test.aic} | BMqs: ${test.bmqs} | Buffer: ${test.bufferMaintained ? '✅' : '❌'}`).join('\n')}
...
async function processAndDisplay(input, source) {
    try {
        ...
        if (result.error) {
            vscode.window.showErrorMessage(`🔥 BLF Heat Shield: ${result.error}`, 'Show Details').then(selection => {
                if (selection === 'Show Details') {
                    outputChannel.show();
                }
            });
            ...
            vscode.window.showInformationMessage(`🎯 BLF Processed: AIC=${result.aic}, BMqs=${result.bmqs} | ${result.status}`, 'Show Details').then(selection => {
                if (selection === 'Show Details') {
                    outputChannel.show();
                }
            });
...
function formatBLFResult(result, source) {
    if (result.error) {
